...
void ChatBotFrame::OnEnter(wxCommandEvent &WXUNUSED(event))
{
    ...
    // send user text to chatbot 
     _panelDialog->GetChatLogicHandleSendMessageToChatbot(std::string(userText.mb_str()));
}
...
wxBitmap *ChatBotPanelDialog::GetChatLogicHandleGetImageFromChatbot() {
    return _chatLogic->GetImageFromChatbot();
}

void ChatBotPanelDialog::GetChatLogicHandleSendMessageToChatbot(std::string message) {
    ...
    : wxPanel(parent, -1, wxPoint(-1, -1), wxSize(-1, -1), wxBORDER_NONE)
{
    ...
    wxBitmap *bitmap = isFromUser == true ? nullptr : ((ChatBotPanelDialog*)parent)->GetChatLogicHandleGetImageFromChatbot();
